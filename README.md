...
To use this open up a cockpit in the k-menu and look for the new controls. There are two lists, one for available GPS points and one for active markers. You can create a marker from a properly formatted GPS point using the 'Create from GPS' button, or by double clicking the gps name. In order for a GPS to show in the available list it's name needs to follow some rules:
...
Markers can be toggled on and off in multiple ways. You can use the k-menu terminal controls (from a cockpit) to toggle individual markers or all markers at once. Double click a marker in the list to toggle its active state. You can also use chat commands to toggle individual markers by name or index, or all markers at once.
...
- toggle: Toggle visibility of all markers. Accepts two optional parameters: `<index>/<name>` and `<on/off>`
  - Example: `/nav toggle "Saturn Rings" off`
    - Turns off visibility of the nav marker named "Saturn Rings"
  - Example: `/nav toggle 2 on`
    - Turns on visibility of the nav marker at index 2 from the list command
  - Will first try to find a marker by matching name, then by index if no name match is found and finally by partial match on name if no exact match is found.
...
- load: Loads saved markers from disk
  - Optional parameter of 'true' will create missing GPS markers from the loaded nav markers 
